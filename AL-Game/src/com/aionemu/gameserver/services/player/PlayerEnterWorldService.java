...
public final class PlayerEnterWorldService {
	...
	static {
		...
		String alBuffer = "=============================\n";
		alBuffer = alBuffer + "Dream Aion 5.8.\n";
		alBuffer = alBuffer + "Copyright 2017 Dream Aion\n";
		alBuffer = alBuffer + "=============================\n";
...
//			client.sendPacket(new SM_UNK_154()); // TODO 5.6
			...
			if (player.getClientConnection().getAccount().getMembership() == 2 && player.getLevel() >= 66 && player.getLevel() <= 83) {
				...
				PacketSendUtility.sendBrightYellowMessageOnCenter(player, "Your account Vip ! You get extra Buff !");
			}
			...
			else if (player.getLevel() >= 65 && player.getLevel() <= 83) {
...
//			GloryPointLoseMsg(player);
			...
			MonsterbookService.getInstance().onLogin(player);
			AuthorizeService.getInstance().onSetEquipAccessories(player, EnchantsConfig.MAX_CAP_AUTHORIZED_ACCESORIES);
			AuthorizeService.getInstance().onSetEquipPlume(player, EnchantsConfig.MAX_CAP_AUTHORIZED_ACCESORIES);
		}
		else
