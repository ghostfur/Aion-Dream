 ...
 * @Aion-Core Dev.
 */
package com.aionemu.gameserver.services.thedevils.oneVsone;
...
public class ArenaMasterService{
    ...
    static Point3D loc1 = new Point3D(SempakConfig.ONEVSONE_POINTX_1, SempakConfig.ONEVSONE_POINTY_1, SempakConfig.ONEVSONE_POINTZ_1); // done config  //winer point++
    static Point3D loc2 = new Point3D(SempakConfig.ONEVSONE_POINTX_2, SempakConfig.ONEVSONE_POINTY_2, SempakConfig.ONEVSONE_POINTZ_2); // done config //loser point++
    static Point3D loc21 = new Point3D(SempakConfig.ONEVSONE_POINT2X_1, SempakConfig.ONEVSONE_POINT2Y_1, SempakConfig.ONEVSONE_POINT2Z_1);
    static Point3D loc22 = new Point3D(SempakConfig.ONEVSONE_POINT2X_2, SempakConfig.ONEVSONE_POINT2Y_2, SempakConfig.ONEVSONE_POINT2Z_2);
    static Point3D loc31 = new Point3D(SempakConfig.ONEVSONE_POINT3X_1, SempakConfig.ONEVSONE_POINT3Y_1, SempakConfig.ONEVSONE_POINT3Z_1);
    static Point3D loc32 = new Point3D(SempakConfig.ONEVSONE_POINT3X_2, SempakConfig.ONEVSONE_POINT3Y_2, SempakConfig.ONEVSONE_POINT3Z_2);
    public String ArenaMaster = SempakConfig.ONEVSONE_ARENAMASTER; // make config+++
    public int item1 = SempakConfig.ITEM1; //make config++
    public int item2 = SempakConfig.ITEM2; //make config++
    public int quantity1 = SempakConfig.QUANTITY1; //make config++
    public int quantity2 = SempakConfig.QUANTITY2; // make config++
    ...
    public void onLeaveInstance(Player player){
        if(!SempakConfig.ENABLE_ONEVONE){
            return;
        }
        for (int world : OneVsOneStruct.worldid){
            if(player.getWorldId() != world){
    ...
    private void giveRewards(Player winner, Player loser){ //make config for AP, GP , or ITEM REWARD
        ...
        if(winz.getWinCount() == lost.getWinCount()){ // if its a tie, give lose AP/GP reward for both, no ItemReward and no toll reward
            ...
            if(SempakConfig.ENABLE_AP_REWARD){
                AbyssPointsService.addAp(winz, SempakConfig.AP_REWARD / 2);
                AbyssPointsService.addAp(lost, SempakConfig.AP_REWARD / 2);
            }
            if(SempakConfig.ENABLE_GP_REWARD){
                AbyssPointsService.addGp(winz, SempakConfig.GP_REWARD / 2);
                AbyssPointsService.addGp(lost, SempakConfig.GP_REWARD / 2);
            }
            return;
        }

        if (SempakConfig.ITEMREWARD_ENABLE){ // The item reward is not a 100% reward that you get, there is chance that you will get it or not
            if(rnd == 18){
        ...
        if(SempakConfig.ENABLE_AP_REWARD){
            AbyssPointsService.addAp(winz, SempakConfig.AP_REWARD);
            AbyssPointsService.addAp(lost, SempakConfig.AP_REWARD / 2);
        }
        if(SempakConfig.ENABLE_GP_REWARD){
            AbyssPointsService.addGp(winz, SempakConfig.GP_REWARD);
            AbyssPointsService.addGp(lost, SempakConfig.GP_REWARD / 2);
        }
        /*if(SempakConfig.ENABLE_TOLL_REWARD){
            InGameShopEn.getInstance().addToll(winz, SempakConfig.TOLL_REWARD);
            InGameShopEn.getInstance().addToll(lost, SempakConfig.TOLL_REWARD / 2);
        }*/
    }
    ...
    public void teleportIn(Player player1, Player player2) {
        int random = Rnd.get(1, SempakConfig.ONEVSONE_MAPCNT);
    ...
    private void getReady(Player p1, Player p2){ // add removecd without xform skills
        ...
        if (SempakConfig.ENABLE_REMOVECD){
            resetSkills(p1);
        }
    ...
    private void resetSkills(Player player1){
        ...
        if (player1.getSkillCoolDowns() != null) {
            ...
            for (Map.Entry<Integer, Long> en : player1.getSkillCoolDowns().entrySet()) {
                ...
                if (!SempakConfig.TOUCH_XFORM_SKILL){
