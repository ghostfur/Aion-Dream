...
public class Equipment {
	...
	public Item equipItem(int itemUniqueId, long slot) {
		...
		int itemLevel = item.getItemTemplate().getLevel() - item.getReductionLevel();
		if (itemLevel > owner.getLevel() && item.getItemTemplate().getLevel() < 66) {
			PacketSendUtility.sendPacket(this.owner, SM_SYSTEM_MESSAGE.STR_CANNOT_USE_ITEM_TOO_LOW_LEVEL_MUST_BE_THIS_LEVEL(item.getNameId(), itemLevel));
			return null;
		}
	...
	public FastList<Item> getEquippedAllItemsWithoutStigma() {
