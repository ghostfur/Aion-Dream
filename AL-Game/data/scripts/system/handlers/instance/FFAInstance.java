...
import com.aionemu.gameserver.services.thedevils.ffa.FFaStruct;
...
public class FFAInstance extends GeneralInstanceHandler {
    ...
    static Point3D[] positions = new Point3D[]{
            new Point3D(SempakConfig.FFA_SPAWNPOINT_1X, SempakConfig.FFA_SPAWNPOINT_1Y,SempakConfig.FFA_SPAWNPOINT_1Z),
            new Point3D(SempakConfig.FFA_SPAWNPOINT_2X,SempakConfig.FFA_SPAWNPOINT_2Y,SempakConfig.FFA_SPAWNPOINT_2Z),
            new Point3D(SempakConfig.FFA_SPAWNPOINT_3X,SempakConfig.FFA_SPAWNPOINT_3Y,SempakConfig.FFA_SPAWNPOINT_3Z),
            new Point3D(SempakConfig.FFA_SPAWNPOINT_4X,SempakConfig.FFA_SPAWNPOINT_4Y,SempakConfig.FFA_SPAWNPOINT_4Z),
            new Point3D(SempakConfig.FFA_SPAWNPOINT_5X,SempakConfig.FFA_SPAWNPOINT_5Y,SempakConfig.FFA_SPAWNPOINT_5Z),
            new Point3D(SempakConfig.FFA_SPAWNPOINT_6X,SempakConfig.FFA_SPAWNPOINT_6Y,SempakConfig.FFA_SPAWNPOINT_6Z),
            new Point3D(SempakConfig.FFA_SPAWNPOINT_7X,SempakConfig.FFA_SPAWNPOINT_7Y,SempakConfig.FFA_SPAWNPOINT_7Z),
            new Point3D(SempakConfig.FFA_SPAWNPOINT_8X,SempakConfig.FFA_SPAWNPOINT_8Y,SempakConfig.FFA_SPAWNPOINT_8Z),
            new Point3D(SempakConfig.FFA_SPAWNPOINT_9X,SempakConfig.FFA_SPAWNPOINT_9Y,SempakConfig.FFA_SPAWNPOINT_9Z),
            new Point3D(SempakConfig.FFA_SPAWNPOINT_10X,SempakConfig.FFA_SPAWNPOINT_10Y,SempakConfig.FFA_SPAWNPOINT_10Z),
    };
...
// CHECK IF DOUBLE INSTANCE CREATE WHEN DONE ON SUPER.ONSTANCE - DOne
    ...
    public void onEnterInstance(Player player){
        PacketSendUtility.sendSys2Message(player, SempakConfig.FFA_ANNOUNCER_NAME, SempakConfig.FFA_WELCOME_MSG);
    ...
    public void onLeaveInstance(Player player){
        ...
        player.setSpecialKills(0);
        player.setKSLevel(0);
        PacketSendUtility.sendSys2Message(player, SempakConfig.FFA_ANNOUNCER_NAME, SempakConfig.FFA_LEAVE_MSG);
    }
    ...
    public void checkKillReward(final Player winner, boolean isAp, boolean isGp){
        ...
        if(winner.getSpecialKills() <= 5){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP1);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP1);
            }
        }else if(winner.getSpecialKills() <= 10){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP2);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP2);
            }
        }else if(winner.getSpecialKills() <=15){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP3);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP3);
            }
        }else if(winner.getSpecialKills() <=20){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP4);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP4);
            }
        }else if(winner.getSpecialKills() <=25){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP5);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP5);
            }
        }else if(winner.getSpecialKills() <=30){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP6);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP6);
            }
        }else if(winner.getSpecialKills() <=35){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP7);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP7);
            }
        }else if(winner.getSpecialKills() <=40){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP8);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP8);
            }
        }else if(winner.getSpecialKills() <=45){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP9);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP9);
            }
        }else if(winner.getSpecialKills() <=50){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP10);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP10);
            }
        }else if(winner.getSpecialKills() > 50){
            if(isAp){
                AbyssPointsService.addAp(winner, SempakConfig.FFA_REWARD_AP11);
            }else if(isGp){
                AbyssPointsService.addGp(winner, SempakConfig.FFA_REWARD_GP11);
            }
        }
...
//MAKE MANUAL SK KILL SET IN .FFA
    public void giveKillMsgWithKills(Player winner, Player loser){
        ...
        if(updatedKillStreak == 5){
            ...
            kSLevel = 1;
            log.info(SempakConfig.FFA_ANNOUNCER_NAME + " " + winner.getName() + " is on a Killing Spree of Level " + kSLevel);
    ...
    public void checkIfKillingSpree(Player oneWithKS, Player ksEnter){
        ...
        if(kSLevel == 1){
            ...
            log.info(SempakConfig.FFA_ANNOUNCER_NAME + " " + oneWithKS.getName() + "'s killing spree has been ended by " + ksEnter.getName());
        }else if(kSLevel == 2){
    ...
    public void giveSomeHelp(Player winner){
        winner.getLifeStats().setCurrentHp(winner.getLifeStats().getCurrentHp() + SempakConfig.FFA_KILL_BONUS_HP);
        winner.getLifeStats().setCurrentMp(winner.getLifeStats().getCurrentMp() + SempakConfig.FFA_KILL_BONUS_MP);
        winner.getCommonData().addDp(SempakConfig.FFA_KILL_BONUS_DP);
    }
    ...
    public void KillingSpreeMsg(final String msg){
        World.getInstance().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player object) {
                ...
                PacketSendUtility.sendSys2Message(object, SempakConfig.FFA_ANNOUNCER_NAME , msg);
            }
        });
    }
    ...
    public void announceNormalKillMsg(final Player winner,final Player loser, final String msg){
        instance.doOnAllPlayers(new Visitor<Player>() {
            @Override
            public void visit(Player object) {
                PacketSendUtility.sendSys2Message(object, SempakConfig.FFA_ANNOUNCER_NAME, winner.getName() + " " + msg + " " + loser.getName());
            }
        });
    }
